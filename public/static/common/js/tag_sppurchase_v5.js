...
function shop_add_cart() {
    ...
    if (parseInt(StockCountObj) == 0) {
        if (!window.layer) {
            ...
            layer.alert('The product is sold out!', {icon: 5, title: false, closeBtn: false});
        }
        return false;
        ...
        if (!window.layer) {
            ...
            layer.alert('Insufficient inventory of goods!', {icon: 5, title: false, closeBtn: false});
        }
        return false;
    }
...
function paySelect_v507428(formname)
{
    ...
    content += ' <img src="'+JsonData.root_dir+'/public/static/common/images/alipay.png" width="150" height="50" alt="Alipay payment" />';
    content += '</a>';
    ...
    content += ' <img src="'+JsonData.root_dir+'/public/static/common/images/weipay.png" width="150" height="50" alt="WeChat Pay" />';
    content += '</a>';
    content += '</div>';
...
function OrderPayPolling_v507428(data) {
    ...
    if (!pay_id || !pay_mark || !unified_id || !unified_number || !transaction_type) {
        layer.msg('Order exception 005, refresh and retry', {time: 1500}, function(){
...
function CartUnifiedAlgorithm(symbol){
    ...
    if (StockCountObj.value < quantity) {
        if (!window.layer) {
            alert('The inventory of this product is only:'+StockCountObj.value);
        } else {
            layer.alert('The inventory of this product is only:'+StockCountObj.value, {icon: 5, title: false, closeBtn: false});
        }
